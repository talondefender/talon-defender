...
const CORE_DEFAULT_RULESET_IDS = [
    'ublock-filters',
    'easylist',
    'easyprivacy',
    'ublock-badware',
...
export async function getDefaultRulesetsFromEnv() {
    ...
    const available = new Set(rulesetDetails.keys());
    return CORE_DEFAULT_RULESET_IDS.filter(id => available.has(id));
}
...
async function patchDefaultRulesets() {
    ...
    const toAdd = [];
    const toRemove = [];
    const allowedIds = new Set(newDefaultIds);
