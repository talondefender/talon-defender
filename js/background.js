...
const normalizeHttpHostname = (url) => {
...
const getDowngradedFilteringMode = (level) => {
...
const applyAutoBackoff = async (hostname) => {
    ...
    const targetLevel = getDowngradedFilteringMode(beforeLevel);
...
function isEntitled() {
    return entitlementStatus?.status !== 'expired';
}
...
async function scheduleTrialExpiredReminderAlarm(status) {
    ...
    let when = now + TRIAL_EXPIRED_REMINDER_INITIAL_DELAY_MS;
    if (storedLastShown > 0) {
        when = storedLastShown + TRIAL_EXPIRED_REMINDER_INTERVAL_MS;
    }
    if (Number.isFinite(when) === false || when <= now) {
        when = now + TRIAL_EXPIRED_REMINDER_INITIAL_DELAY_MS;
    }
...
async function maybeShowTrialExpiredReminder() {
    ...
    await gotoURL(url).catch(ubolErr);
...
function onMessage(request, sender, callback) {
    ...
    switch (what) {
        ...
        case 'setLicenseKey': {
            const licenseKey = typeof request.licenseKey === 'string'
                ? request.licenseKey.trim()
                : '';
            if (licenseKey === '' || licenseKey.length > MAX_LICENSE_KEY_LENGTH) {
                callback({ error: 'invalid_license_key' });
                return true;
            }
            storeLicenseKey(licenseKey).then(() =>
