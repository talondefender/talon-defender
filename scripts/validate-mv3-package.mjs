...
const checkPackagedComplianceFiles = async () => {
  const requiredFiles = [
    ...
    'js/reload-sensitive-artifact-meta.js',
...
const RELOAD_SENSITIVE_RESOURCE_RE =
  /^(?:js\/scripting\/scriptlet-token|rulesets\/scripting\/scriptlet)\/[a-z0-9._/-]+\.js$/i;

const sha256Hex = bytes => createHash('sha256').update(bytes).digest('hex');
const sha256JavaScript = bytes => sha256Hex(Buffer.from(
  bytes.toString('utf8').replace(/\r\n?/g, '\n'),
  'utf8'
));

const checkReloadSensitiveArtifactManifest = async () => {
  ...
  const metadataPath = path.join(targetDir, metadataRelativePath);
  if (
    await pathExists(manifestPath) === false ||
    await pathExists(metadataPath) === false
  ) {
    return;
  }
  ...
  let metadataSource;
  try {
    [ artifactManifest, metadataSource ] = await Promise.all([
    ...
    addViolation(
      ...
      'Reload-sensitive artifact metadata is unreadable or invalid JSON'
    );
    return;
  }
  const resources = artifactManifest?.resources;
  if (
    artifactManifest?.version !== 1 ||
    resources instanceof Object === false ||
    Array.isArray(resources)
  ) {
    addViolation(
      manifestRelativePath,
      'Reload-sensitive artifact manifest must use schema version 1 with a resource map'
    );
    return;
  }
  const entries = Object.entries(resources);
  const resourcePaths = entries.map(([ resourcePath ]) => resourcePath);
  if (
    entries.length === 0 ||
    JSON.stringify(resourcePaths) !== JSON.stringify(resourcePaths.slice().sort())
  ) {
    addViolation(
      manifestRelativePath,
      'Reload-sensitive artifact resources must be non-empty and sorted by path'
    );
  }
  for (const [ resourcePath, digest ] of entries) {
    const relativePath = typeof resourcePath === 'string' && resourcePath.startsWith('/')
      ? resourcePath.slice(1)
      : '';
    if (
      relativePath === '' ||
      RELOAD_SENSITIVE_RESOURCE_RE.test(relativePath) === false ||
      ...
      addViolation(
        manifestRelativePath,
        `Invalid reload-sensitive artifact entry: ${String(resourcePath)}`
      );
      continue;
    }
    const packagedPath = path.join(targetDir, relativePath);
    if (await pathExists(packagedPath) === false) {
      addViolation(
        relativePath,
        'Reload-sensitive artifact listed in the digest manifest is missing from the package'
      );
      continue;
    }
    const actualDigest = sha256JavaScript(await fs.readFile(packagedPath));
    if (actualDigest !== digest) {
      addViolation(
        relativePath,
        'Packaged reload-sensitive asset does not match its generated SHA-256 digest'
      );
    }
  }

  const computedFingerprint = sha256Hex(Buffer.from(JSON.stringify(resources), 'utf8'));
  if (artifactManifest.fingerprint !== computedFingerprint) {
    addViolation(
      manifestRelativePath,
      'Reload-sensitive artifact manifest fingerprint does not match its resource map'
    );
  }
  const metadataFingerprint = metadataSource.match(
    /RELOAD_SENSITIVE_ARTIFACT_MANIFEST_FINGERPRINT\s*=\s*'([0-9a-f]{64})'/
  )?.[1];
  const expectedMetadata =
    ...
    `export const RELOAD_SENSITIVE_ARTIFACT_MANIFEST_FINGERPRINT = '${computedFingerprint}';\n` +
    "export const RELOAD_SENSITIVE_ARTIFACT_MANIFEST_PATH = '/rulesets/reload-sensitive-artifacts.json';\n";
  if (
    metadataFingerprint !== computedFingerprint ||
    metadataSource.replaceAll('\r\n', '\n') !== expectedMetadata
  ) {
    addViolation(
      metadataRelativePath,
      'Reload-sensitive artifact metadata version, path, or fingerprint does not match the generated manifest'
    );
  }

  const packagedFiles = await collectFiles(targetDir);
  const packagedReloadSensitivePaths = packagedFiles
    .map(getRelPath)
    .filter(relativePath => RELOAD_SENSITIVE_RESOURCE_RE.test(relativePath))
    .sort();
  const manifestPathSet = new Set(resourcePaths.map(resourcePath =>
    typeof resourcePath === 'string' && resourcePath.startsWith('/')
      ? resourcePath.slice(1)
      : ''
  ));
  for (const relativePath of packagedReloadSensitivePaths) {
    if (manifestPathSet.has(relativePath)) { continue; }
    addViolation(
      relativePath,
      'Packaged reload-sensitive asset is missing from the generated digest manifest'
    );
  }
};
...
const main = async () => {
  ...
  await checkReloadSensitiveArtifactManifest();
