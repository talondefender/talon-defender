...
- YouTube uses a Talon-owned playback guard and narrowed ad-skip lane on YouTube hosts only. The guard leaves YouTube player response ad metadata intact, shields only known playback enforcement/reset signals, keeps playback-wall reload recovery disabled, and prefers bounded synthetic Skip/Skip Ads control activation over speed-through playback. uBO Lite scriptlet/runtime parity is intentionally excluded on YouTube hosts so Talon does not copy uBO Lite's YouTube method.
