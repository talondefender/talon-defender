...
const youtubeServerContractReplacement = String.raw`(()=>{
...
const originalUserAgent=ytcfg.data_.INNERTUBE_CONTEXT.client.userAgent,setUserAgent=value=>{ytcfg.data_.INNERTUBE_CONTEXT.client.userAgent=value?originalUserAgent.replace?.(/(Mozilla\/5\.0 \([^)]+)/,"$1; "+value):originalUserAgent},fallbackUserAgents=["channel"],sanitizePlayerResponse=(value,seen=new WeakSet())=>{
    if(!value||"object"!=typeof value||seen.has(value))return value;
    seen.add(value);
    if(Array.isArray(value)){
        for(const entry of value)sanitizePlayerResponse(entry,seen);
        return value;
    }
    let hadAdSlots=!1;
    try{hadAdSlots="adSlots"in value}catch{}
    if(hadAdSlots){
        try{delete value.adSlots}catch{}
        try{value.no_ads=value.no_ads||{}}catch{}
    }
    if("raw_player_response"in value&&"string"==typeof value.raw_player_response&&value.raw_player_response.includes("adSlots"))try{const parsed=JSON.parse(value.raw_player_response);sanitizePlayerResponse(parsed,seen),value.raw_player_response=JSON.stringify(parsed)}catch{}
    if("player_response"in value&&"string"==typeof value.player_response&&value.player_response.includes("adSlots"))try{const parsed=JSON.parse(value.player_response);sanitizePlayerResponse(parsed,seen),value.player_response=JSON.stringify(parsed)}catch{}
    for(const entry of Object.values(value))sanitizePlayerResponse(entry,seen);
    return value;
};
let initialPlayerResponse=sanitizePlayerResponse(window.ytInitialPlayerResponse||{}),fallbackPending=!1,remainingFallbackUserAgents=fallbackUserAgents;
try{
    Object.defineProperty(window,"ytInitialPlayerResponse",{configurable:!0,enumerable:!0,get:()=>initialPlayerResponse,set:value=>{initialPlayerResponse=sanitizePlayerResponse(value)}});
}catch{window.ytInitialPlayerResponse=initialPlayerResponse}
document.addEventListener("DOMContentLoaded",(function(){
    const evaluateStartup=()=>{
        const player=document.getElementById("movie_player");
        if(!player||!window.location.href.includes("/watch?"))return void(remainingFallbackUserAgents=fallbackUserAgents);
        const playerResponse=sanitizePlayerResponse(player.getPlayerResponse?.()),progressState=player.getProgressState?.(),stats=player.getStatsForNerds?.();
        if(progressState&&progressState.duration>0&&(progressState.loaded<progressState.duration||progressState.duration-progressState.current>1)||playerResponse?.videoDetails?.isLive){
            if(!stats?.debug_info?.startsWith?.("SSAP, AD")){
                const videoId=playerResponse?.videoDetails?.videoId,startSeconds=playerResponse?.playerConfig?.playbackStartConfig?.startSeconds??0,isBuffering=player.getPlayerStateObject?.()?.isBuffering;
                return void("UNPLAYABLE"!==playerResponse?.playabilityStatus?.status||playerResponse?.playabilityStatus?.errorScreen?.playerErrorMessageRenderer?.playerCaptchaViewModel||"WEB_PAGE_TYPE_UNKNOWN"!==playerResponse?.playabilityStatus?.errorScreen?.playerErrorMessageRenderer?.subreason?.runs?.[0]?.navigationEndpoint?.commandMetadata?.webCommandMetadata?.webPageType||"https://support.google.com/youtube/answer/3037019"!==playerResponse?.playabilityStatus?.errorScreen?.playerErrorMessageRenderer?.subreason?.runs?.[0]?.navigationEndpoint?.commandMetadata?.webCommandMetadata?.url?0===remainingFallbackUserAgents.length?(fallbackPending=!1,setUserAgent("")):isBuffering&&"0.00 s"===stats?.buffer_health_seconds&&"0x0"===stats?.resolution&&fallbackPending&&(setUserAgent(remainingFallbackUserAgents[0]),fallbackPending=!1,player.loadVideoById(videoId,startSeconds)):(remainingFallbackUserAgents=remainingFallbackUserAgents.slice(1),remainingFallbackUserAgents.length>0?setUserAgent(remainingFallbackUserAgents[0]):setUserAgent(""),fallbackPending=!1,player.loadVideoById(videoId,startSeconds)));
            }
            progressState.duration>0&&player.seekTo?.(progressState.duration);
        }
    };
    evaluateStartup();
    new MutationObserver((()=>{evaluateStartup()})).observe(document,{childList:!0,subtree:!0});
}));
window.Map.prototype.has=new Proxy(window.Map.prototype.has,{apply:(target,thisArg,args)=>{
    if("onSnackbarMessage"===args?.[0]&&!fallbackPending){
        const player=document.getElementById("movie_player");
        if(!player)return;
        const stats=player.getStatsForNerds?.(),isBuffering=player.getPlayerStateObject?.()?.isBuffering,playbackBaseUrl=sanitizePlayerResponse(player.getPlayerResponse?.())?.playbackTracking?.videostatsPlaybackUrl?.baseUrl;
        isBuffering&&"0.00 s"===stats?.buffer_health_seconds&&"0x0"===stats?.resolution&&remainingFallbackUserAgents.length>0&&(playbackBaseUrl?.includes?.("reloadxhr")&&(remainingFallbackUserAgents=remainingFallbackUserAgents.slice(1)),fallbackPending=!0);
    }
    return Reflect.apply(target,thisArg,args);
}});
const promiseThenProxy={apply:(target,thisArg,args)=>{const callback=args[0];return"function"==typeof callback&&callback.toString().includes("onAbnormalityDetected")&&(args[0]=function(){}),Reflect.apply(target,thisArg,args)}};
window.Promise.prototype.then=new Proxy(window.Promise.prototype.then,promiseThenProxy);
const parseProxy={apply:(target,thisArg,args)=>{let parsed=sanitizePlayerResponse(Reflect.apply(target,thisArg,args));const videoId=parsed?.videoDetails?.videoId;return videoId&&location.search.includes(videoId)&&window.ytInitialPlayerResponse?.storyboards?.playerStoryboardSpecRenderer&&(parsed.storyboards=window.ytInitialPlayerResponse.storyboards,window.ytInitialPlayerResponse.storyboards={}),parsed}};
window.JSON.parse=new Proxy(window.JSON.parse,parseProxy);
})();(function serverContract()`;
argsList[0] = [
    "script",
    "(function serverContract()",
    youtubeServerContractReplacement,
    "sedCount",
    "1",
];
