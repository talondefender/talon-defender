...
- `talon-site-fixes` carries a narrow French Stream playback fix for `french-stream.one`/known embedded providers: it blocks the observed third-party click-hijack loader and landing chains through `dp.humpingunfoggy.cfd`, `vascon.kalesrussiaschuln.cyou`, `thawier.beholdsresinsimprevu.cfd`, `hoinsealch.qpon`, `fsurl.lol` hidden SSO frames, `bulbedcrus.shop`, `teindpumpage.cfd`, `scrougespongesgutsy.cyou`, `keylesshowk.shop`, `fartingpangane.shop`, `073m.com`, `v2006.com`, `2osb.com`, `webls.net`, `phiglerdail.net`, `wvdme.com`, and the redirected AliExpress landing path; hides `#dontfoid`; blocks base-target, opener, and creative postMessage popup abuse during top-page and player gestures; suppresses unsolicited provider `enter_fullscreen` bridge messages unless they follow a trusted fullscreen-control click; removes the hidden startup SSO iframe that caused post-load player flicker; and leaves the provider frame, media controls, seeking, and video fullscreen available. French Stream now also has a static isolated bootstrap content script that wakes the background, respects entitlement/paywall checks, restores dynamic registrations when needed, and injects the existing page-world site-fix into the current frame so first-page visits are protected even when dynamic registration was not ready at browser startup.
