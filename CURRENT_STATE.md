...
- signed public tactics now accept only exact-host `jsonPrune` and `jsonSet` entries for same-origin JSON `fetch` or `XMLHttpRequest` responses, reject Talon-owned and protected-domain targets, cap baseline/overlay/compiled counts, and drop compiled overflow from the tail after overlay-first precedence
...
- the packaged remote-tactics lane now registers one isolated-world bootstrap plus one MAIN-world interpreter at `document_start`, installs wrappers before page code runs, bridges exact-host tactic config by `CustomEvent`, and fails closed back to the original response on parse, bridge, or tactic errors
