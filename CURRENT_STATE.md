...
- YouTube now uses a Talon-owned MV3 ad-skip/runtime guard lane on `youtube.com` and `youtube-nocookie.com`. It is dynamically registered behind entitlement/injectable sync, clicks visible skip controls, hides YouTube ad surfaces, suppresses YouTube's matching "Experiencing interruptions" notice, temporarily mutes and accelerates ad video playback, restores the user's video mute/playback-rate state after the ad clears, prunes player ad metadata on player-response paths, limits JSON.parse pruning to Shorts ad entries, suppresses YouTube `onAbnormalityDetected` reset callbacks, and only corrects SSAP restart loops when YouTube's SSAP experiment flag and armed range shape are present.
