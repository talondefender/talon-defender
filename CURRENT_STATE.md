...
- YouTube now uses a Talon-owned MV3 playback guard and narrowed ad-skip lane on `youtube.com` and `youtube-nocookie.com`. The guard is installed at document start, leaves YouTube player response ad metadata intact, suppresses known abnormality/reset callbacks, shields only targeted playback enforcement storage signals, accelerates YouTube's native 17-second detector timer, bridges the guarded fetch into new `about:blank` frames, and only installs the SSAP restart-loop guard when YouTube's SSAP experiment flag and armed range shape are present. The ad-skip lane no longer performs synthetic skip-button clicks or broad YouTube ad-surface hiding; it only detects the player's native ad-showing state, temporarily mutes and accelerates ad video playback, restores the user's video mute/playback-rate state after the ad clears, coalesces YouTube DOM mutation bursts, watches only the player class for fast ad-state transitions, and suppresses YouTube's matching "Experiencing interruptions" notice.
