...
- YouTube now uses a Talon-owned MV3 ad-skip/runtime guard lane on `youtube.com` and `youtube-nocookie.com`. It is dynamically registered behind entitlement/injectable sync, clicks visible skip controls, hides YouTube ad surfaces, suppresses YouTube's matching "Experiencing interruptions" notice, immediately hides newly inserted matching interruption notices, temporarily mutes and accelerates ad video playback, restores the user's video mute/playback-rate state after the ad clears, coalesces YouTube DOM mutation bursts before rescanning ad surfaces during SPA navigation, watches only the player class for fast ad-state transitions, rate-limits full-page interruption-notice text scans, lazily sanitizes player fetch response bodies only when YouTube reads `json()`, `text()`, or a clone, limits JSON.parse pruning to Shorts ad entries, suppresses YouTube `onAbnormalityDetected` reset callbacks, and only installs and applies the SSAP restart-loop guard when YouTube's SSAP experiment flag and armed range shape are present.
