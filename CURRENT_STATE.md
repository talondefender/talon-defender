...
- YouTube now uses a Talon-owned MV3 playback guard and narrowed ad-skip lane on `youtube.com` and `youtube-nocookie.com`. The guard is installed at document start, leaves YouTube player response ad metadata intact, shields only targeted playback enforcement storage signals and named abnormality callbacks, leaves YouTube's native detector timers untouched, uses bounded scheduled playback-wall checks instead of broad mutation-driven body scans, and keeps DOM append and Array push hooks opt-in rather than default. The ad-skip lane no longer performs synthetic skip-button clicks, broad YouTube ad-surface hiding, or whole-document mutation observation; it detects the player's native ad-showing state, temporarily mutes and accelerates native ad video playback, restores the user's video mute/playback-rate state after the ad clears, watches only the player class for fast ad-state transitions, and suppresses YouTube's matching "Experiencing interruptions" notice through timer and navigation checks.
...
- default rulesets enabled in the manifest are `ublock-filters`, `talon-youtube-allow`, `talon-site-fixes`, `easylist`, `easyprivacy`, `pgl`, `ublock-badware`, and `urlhaus-full`
- `talon-site-fixes` carries a narrow French Stream playback fix for `french-stream.one`/known embedded providers: it blocks the observed third-party click-hijack loader and landing chains through `dp.humpingunfoggy.cfd`, `vascon.kalesrussiaschuln.cyou`, `thawier.beholdsresinsimprevu.cfd`, `hoinsealch.qpon`, `bancusnonpeak.cyou`, `domingbesnarecrex.qpon`, `devilyquondam.cyou`, `haivamtuzeton.site`, `fsurl.lol` hidden SSO frames, `bulbedcrus.shop`, `teindpumpage.cfd`, `scrougespongesgutsy.cyou`, `keylesshowk.shop`, `fartingpangane.shop`, `073m.com`, `v2006.com`, `2osb.com`, `webls.net`, `phiglerdail.net`, `wvdme.com`, the redirected AliExpress landing path, and the observed SHEIN, Gaming America, Newsuggest, Endfield, best.aliexpress.com, and Once Human landings when launched from the French Stream ad chain; adds French Stream-scoped third-party fallbacks for rotating `.cyou`, `.qpon`, `.shop`, and `.cfd` ad hosts; hides `#dontfoid`; blocks base-target, opener, direct target assignment on popup-capable navigation elements, and creative postMessage popup abuse during top-page and player gestures; shields trusted same-origin French Stream content-link clicks from site-level ad handlers while preserving normal link navigation; suppresses unsolicited provider `enter_fullscreen` bridge messages unless they follow a trusted fullscreen-control click; removes the hidden startup SSO iframe that caused post-load player flicker; and leaves the provider frame, media controls, seeking, and video fullscreen available. French Stream now ships the page-world guard as a static `document_start` content script on `french-stream.one` and the known player frames so popup APIs are wrapped before site scripts run, while the isolated bootstrap still wakes the background, respects entitlement/paywall checks, and restores dynamic registrations when needed. The Talon site-fix rules are also mirrored into a dedicated runtime DNR range on startup so unpacked/dev Chrome profiles cannot keep using a stale compiled static ruleset cache after the JSON changes without a manifest version bump.
