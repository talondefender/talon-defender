...
- YouTube now uses a Talon-owned MV3 playback guard and narrowed ad-skip lane on `youtube.com` and `youtube-nocookie.com`. The guard is installed at document start, leaves YouTube player response ad metadata intact, shields only targeted playback enforcement storage signals, accelerates YouTube's native 17-second detector timer, uses bounded scheduled playback-wall checks instead of broad mutation-driven body scans, and keeps global Promise, DOM append, and Array push hooks opt-in rather than default. The ad-skip lane no longer performs synthetic skip-button clicks, broad YouTube ad-surface hiding, or whole-document mutation observation; it detects the player's native ad-showing state, temporarily mutes and accelerates ad video playback, restores the user's video mute/playback-rate state after the ad clears, watches only the player class for fast ad-state transitions, and suppresses YouTube's matching "Experiencing interruptions" notice through timer and navigation checks.
